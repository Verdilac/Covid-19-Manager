...
import { Modal, Segment, Button, Form, Table } from "semantic-ui-react";
...
const ProductTable = () => {
  ...
  const emptyRows =
    rowsPerPage -
    Math.min(rowsPerPage, productData.length - page * rowsPerPage);
  ...
  const handleFormValidations = () => {
    ...
    if (uProductName.trim().length == 0) {
      productNameError.productNameNone =
        "The product name field cannot be empty";
      isValid = false;
    }
    ...
    if (uProductDescription.trim().length == 0) {
      productDescriptionError.productDescNone =
        "The product description field cannot be empty";
      isValid = false;
    }

    if (uProductDescription.trim().length > 30) {
      productDescriptionError.productDescLong =
    ...
    if (uProductStatus.trim().length == 0) {
      productStatusError.productStatusNone =
        "The product status field cannot be empty";
      isValid = false;
    }
    ...
    if (!uProductStatus.trim().length == 0) {
      ...
      if (
        !status_words.some((word) =>
          uProductStatus.toUpperCase().startsWith(word)
        )
      ) {
        productStatusError.productStatusNames =
          "Hint: Product Status should be either Ordered, InStock or UnAvailable";
        isValid = false;
      }
    }
    ...
    if (uProductExpiration.trim().length == 0) {
      productExpirationError.productExpirationNone =
        "An Expiration Date must be specified";
      isValid = false;
    }

    // 5) Product Quantity Validations
    if (uProductQuantity.trim().length == 0) {
      productQuantityError.productQuantityNone =
        "The product quantity field cannot be empty";
      isValid = false;
    }
    ...
    if (!uProductQuantity.trim().length == 0) {
      ...
      if (!Number.isInteger(Number(uProductQuantity.trim()))) {
        productQuantityError.productQuantityNone =
          "The product quantity should be an integer value";
        isValid = false;
      }

      if (
        Number(uProductQuantity.trim()) == 0 ||
        Number(uProductQuantity.trim()) < 0
      ) {
        productQuantityError.productQuantityZeroOrNegative =
          "Hint: Quanity should be greater than zero";
        isValid = false;
      }
    ...
    <div>
      ...
      <Segment className="I_product_table_segment">
        ...
        <Table fixed singleLine className="I_product_table">
          <Table.Header>
            <Table.Row>
              <Table.HeaderCell className="I_product_table_header_cell">
                {" "}
                Product Name{" "}
              </Table.HeaderCell>
              <Table.HeaderCell className="I_product_table_header_cell">
                        ...
                        <Modal.Content image className="I_update_modal_content">
                          <Modal.Description>
                            ...
                            <Form className="I_in_inset_form">
                              <h1 className="I_update_form_title">
                                Update Product Form
                              </h1>
                              <Form.Field>
                                <label className="I_in_update_form_label">
                                  Edit Product Name
                                </label>
                                <input
                              ...
                              <Form.Field>
                                ...
                                <input
                                  ...
                                  className="I_in_inset_form_label"
                                />
                                {Object.keys(productDescriptionError).map(
                                  (key) => {
                                    return (
                                      <div
                                        style={{
                              ...
                              <Form.Field>
                                ...
                                <input
                                  ...
                                  className="I_in_inset_form_label"
                                />
                                {Object.keys(productExpirationError).map(
                                  (key) => {
                                    return (
                                      <div
                                        style={{
                              ...
                              <Form.Field>
                                ...
                                <input
                                  ...
                                  className="I_in_inset_form_label"
                                />
                                {Object.keys(productQuantityError).map(
                                  (key) => {
                                    return (
                                      <div
                                        style={{
